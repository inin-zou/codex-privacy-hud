...
21. **On an outbound call, the deep scan is best-effort.** The model is serial, so one outbound call scans at a time and asks to wait 1.0 s for it; past that the call proceeds on the fast tiers rather than risk the hook deadline, which I6 would turn into a deny. That 1.0 s is a *requested* timeout and a cutoff for using a result — it does not stop the scan and does not promise the call returns by then (measured: 1.25 s). That fallback can allow what a completed scan would have blocked and leave unmasked what it would have masked — the same as every outbound call before this existed. Each one is recorded, so the session stops reading as fully verified, but the audit cannot tell you which calls they were. ([details](docs/known-limits.md#21-on-an-outbound-call-the-deep-scan-is-best-effort))
