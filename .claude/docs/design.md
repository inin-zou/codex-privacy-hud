...
For the lexical network-file guard introduced in 0.9.1, the denial names the network-command decision and its lexical limitation rather than asserting a file identity, inspected contents or file count. Its only next step is `Run $privacy to review the ledger.` The audit does not identify the referenced file or confirm host enforcement.
