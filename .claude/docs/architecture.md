...
So on B3/B4 the deep scan is admitted one at a time across the daemon, under `engine.TIER3_EGRESS_BUDGET` (1.0 s). **That constant's own comment is the contract, and this document does not restate it** — five reviews found five contradictory paraphrases of it here and elsewhere, every one drifting back toward a promise about elapsed time that the design cannot make. In one line: it is the timeout the caller *requests* and the cutoff a result must beat to be *used*, and it stops neither inference nor the clock (measured: a call under the 1.0 s budget returned at 1.25 s).
...
- **L2/L3** — daemon serves static HTML + vanilla JS on `127.0.0.1:<ephemeral>`; the `$privacy` skill prints the URL and an ASCII table fallback, so the demo works even with no browser.
  - **Which session either surface shows** is resolved by `mcp_tools.resolve_audit_session`: an explicit `$privacy <id>` wins, otherwise the daemon's `active_sessions` op (§2) names the session that fired a hook most recently, and only if the daemon cannot be asked does it fall back to the ledger's most-recently-*started* session — labelled as that, never as the caller's own. Concurrent active sessions are reported, not silently resolved. `local_ui_server`'s default (`/api/session` with no `session_id`) goes through the same function so the page and the ASCII table cannot describe two different sessions, and so does `ambient` — on its own much slower clock (`ambient.RESOLVE_INTERVAL`, ~30 s, against a 2 s redraw), because the identity question is the one thing the ledger cannot answer while the *reading* stays a pure DB poll. All three surfaces therefore name one session at a time. See README known limit 8, including why the ambient line carries no marker for session ambiguity: `⚠unverified` means the record has a hole, not "I am unsure whose record this is", and one glyph cannot carry both.
  - **What the audit header claims** follows from the same resolution: `render.audit(..., resolved=)` writes the subtitle from `ResolvedSession.basis` (`Current session` only for a single daemon-named live session; `Session <id>`, `Most recently active session`, `Most recently started session`, `No session on record` otherwise). Omitting the argument keeps the old constant, so a caller that never resolved cannot acquire a certainty claim it did not earn.
...
**Native status-line item.** `docs/superpowers/specs/2026-09-15-patched-codex-status-line-design.md` moves L1 from the second-pane companion above into Codex's own status line, via a separately built Codex binary. Six units across three contracts, each unit knowing only the contracts drawn between it and its neighbours: **contract A**, `$PLUGIN_DATA/hud/<session_id>.json`, written by the daemon (`HudPublisher`, new) and read by both the patched TUI's `PrivacyStatusSource` and `ambient.py` — no string fields, atomic writes, stale after 30 s, deleted on `SessionEnd`; **contract B**, `HudPublisher.set_hidden(session_id, hidden)`, the sole write path behind the new `$privacy hud on|off|status` MCP tool, so `/statusline` decides whether the item is configured and `$privacy hud` decides whether it currently shows; and **contract C**, `~/.local/share/codex-privacy-hud/manifest.json`, written by `install.sh` and reversed — never inferred — by `install.sh --uninstall`. The six units are: `HudPublisher` (Python, new), `PrivacyStatusSource` and `StatusLineItem::Privacy` (Rust, new, patched into Codex via `patches/privacy-status-line.patch`), the `$privacy hud` MCP tool (extends `mcp_tools.py`), `ambient.py` (unchanged surface, now a fallback rather than *the* L1 surface), and distribution (`scripts/build-patched-codex.sh`, CI release workflow, `install.sh`). The user's official Codex binary is never modified; the patched build is a separate binary installed beside it, and the forwarding script that takes over the `codex` name on `PATH` only chooses between the two by version match. See the spec for the full contract definitions, the Rust patch's four touch points, and the install/uninstall step tables.
