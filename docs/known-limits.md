...
**This is a requested timeout, not a wall-clock guarantee**, and the difference is the thing five review rounds kept finding restated wrongly. 1.0 s is how long the call *asks* to wait, and the cutoff a result has to beat to be used. It does not stop inference, which Python cannot cancel, and it does not decide when the waiting thread resumes, which is the operating system's business: measured, a call under the 1.0 s budget returned at **1.25 s** — timed out correctly, findings discarded, 250 ms after the number. Nothing here bounds the whole request either; the ledger lock, sqlite and the socket are all outside it. Lock contention on the ledger, sqlite and the socket are outside it, and nobody has measured the end-to-end distribution against the client's 2.0 s, so this is a guard against the failure that was measured, not a proof that the hook always answers in time.
