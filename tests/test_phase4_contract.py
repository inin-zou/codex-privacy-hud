...
Every string below is astra's §C text, character for character (the
Chinese included, published as written, never translated here). The
...
RELEASE_SENTENCE = 'Version 0.9.0 activates evidence-based accounting for new sessions observed from a genuine SessionStart, using the selected runtime and fenced ledger introduced in 0.8.0. Existing and late-attached sessions retain legacy accounting; historical records are not backfilled or rescored.'
...
RELEASE_SENTENCE_ZH = '0.9.0 为收到真正 SessionStart 的新会话启用基于证据的记账，并继续使用 0.8.0 引入的运行时选择机制和隔离后的账本。已有会话和开始后才接入的会话仍采用旧版记账；历史记录不会被回填或重新计分。'
...
READ_GUARD = 'The guard remains limited to recognized shell reads and is off by default. Template-file exemptions remain. New accounting distinguishes the file subjects of denied reads, but stores opaque labels rather than sensitive path components. A denial request is not proof that the host stopped the read.'

READ_GUARD_ZH = '读取防护仍只处理能够识别的 shell 读取，默认关闭，并继续豁免模板文件。新版记账会区分被拒绝读取的文件主体，但保存不透明标签，而不是敏感路径片段。发出拒绝请求不能证明宿主已经停止读取。'
