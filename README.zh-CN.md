...
21. **出站调用的深度扫描尽力而为。** 模型串行执行扫描，同一时间只允许一个出站调用进行深度扫描。1.0 秒是调用方请求等待的超时，也是扫描结果被采纳的截止点，不是扫描结束或调用返回的实际耗时保证。Python 无法取消正在运行的 C 扩展调用，推理会继续执行，迟到的结果会被丢弃；超时后，调用仅依据快速检测层的结果继续处理。等待线程何时恢复执行由操作系统决定：实测在 1.0 秒预算下，一次调用到 1.25 秒才返回，正确判定超时并丢弃了结果，但比预算晚了 250 毫秒。账本锁、sqlite 和 socket 操作都在这一预算之外，因此它也不限制整个请求的耗时，不能保证调用总能在 hook 时限内返回、避免被 I6 判为拒绝。这种回退可能放行完整扫描本应拦截的调用，也可能漏掉完整扫描本应脱敏的值——在引入出站深度扫描之前，所有出站调用都是如此。每次深度扫描结果未被采用（从未开始、运行中被放弃或完成得太晚）都会留下记录，因此会话不再显示为已完全验证，但审计无法指出具体是哪些调用。（[详情](docs/known-limits.md#21-on-an-outbound-call-the-deep-scan-is-best-effort)）
