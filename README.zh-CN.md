...
读取防护仍只处理能够识别的 shell 读取，默认关闭，并继续豁免模板文件。新版记账会区分被拒绝读取的文件主体，但保存不透明标签，而不是敏感路径片段。发出拒绝请求不能证明宿主已经停止读取。
...
18. 旧版记录仍可能合并匹配同一模式的文件。新版记账会在所评估路径明确时使用不同的文件主体，并保存不含敏感路径片段的不透明标签。标记统计发出的拒绝；确认读取已停止需要执行证据。（[详情](docs/known-limits.md#18-a-blocked-reads-row-does-not-name-the-file)）
