...
def minimize_tool_input(salt: bytes, tool_name: str, tool_input, findings: list[Finding]):
    ...
    if isinstance(tool_input, str):
        # Silently drop any finding whose span doesn't validate against
        # this particular string (see _matching_findings) rather than
        # raising: findings here may have been collected against a
        # differently-shaped "observation text" than the exact payload
        # being minimized, and a mismatched finding must never corrupt an
        # unrelated slice of outbound text.
    ...
    for key, value in tool_input.items():
        if not isinstance(value, str):
            continue
        field_findings = _matching_findings(value, findings)
        if field_findings:
            new_input[key] = minimize_text(salt, value, field_findings)

    if is_string_tool:
        if isinstance(new_input.get("command"), str):
            return new_input["command"]
        # No "command" field to anchor to (shouldn't happen for real Bash/
        # apply_patch payloads, but never return None here — an unpopulated
        # updated_input on a rewrite decision must never be forwarded to
        # Codex as safe): fall back to a minimized canonical rendering.
